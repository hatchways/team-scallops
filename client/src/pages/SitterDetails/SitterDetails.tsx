...
import { TimePicker, DatePicker, MuiPickersUtilsProvider, KeyboardDatePicker } from '@material-ui/pickers';
...
function SitterDetails(): JSX.Element {
  ...
  const handleSubmit = (
    ...
    const totalPrice = sitterDetails?.profile.ratePerDay
      ? numberOfDays * Number(sitterDetails?.profile.ratePerDay)
      : numberOfDays * 40;

    createRequest(selectedUserSitterId, startDate, endDate, totalPrice, 'WALKING').then((data) => {
      if (data.error) {
